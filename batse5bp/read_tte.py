...
class TTE(object):
    ...
    def __init__(self, fname):
        ...
        for i in range(self.nrows):
            if (i+1) % 32 == 0: print 'Processing row ', i+1, '...'
            row = {}
            for n in range(tot_fields):
                field_name = fields[n]
            ...
            if i == 0:
                ...
                self.tdlist = []
                for j in range(8):
                    if bits[j]: self.tdlist.append(j)
    ...
    def __str__(self):
        ...
        s = s + 'Row data includes: ' + str(self.rows[0].keys()) + '\n'
